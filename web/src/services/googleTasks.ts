/**
 * @file googleTasks.ts — Google Tasks API client (100% reusable from mobile)
 */
...
export async function fetchGoogleTaskLists(accessToken: string): Promise<Array<{ id: string; title: string }>> {
  ...
  if (!response.ok) throw new Error(`Google Tasks lists request failed (${response.status})`);
  const payload = (await response.json()) as GoogleTasksListResponse;
  return payload.items ?? [];
}

export async function fetchGoogleTasks(accessToken: string, taskListId: string): Promise<Task[]> {
...
export async function createGoogleTask(
  ...
  const body: Record<string, unknown> = { title: task.title, notes: task.description };
  if (task.due_date) body.due = new Date(task.due_date).toISOString();
  if (task.status === 'completed') body.status = 'completed';

  const response = await fetch(`${GOOGLE_TASKS_BASE_URL}/lists/${taskListId}/tasks`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
  if (!response.ok) throw new Error(`Create Google Task failed (${response.status})`);
...
export async function updateGoogleTask(
  ...
  const body: Record<string, unknown> = { id: googleTaskId, title: task.title, notes: task.description };
  if (task.due_date) body.due = new Date(task.due_date).toISOString();
  if (task.status) body.status = task.status === 'completed' ? 'completed' : 'needsAction';

  const response = await fetch(`${GOOGLE_TASKS_BASE_URL}/lists/${taskListId}/tasks/${googleTaskId}`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
  if (!response.ok) throw new Error(`Update Google Task failed (${response.status})`);
  return await response.json();
}
...
export async function deleteGoogleTask(
  ...
  if (!response.ok) throw new Error(`Delete Google Task failed (${response.status})`);
}
