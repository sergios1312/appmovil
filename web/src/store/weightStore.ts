/**
 * @file weightStore.ts — Store de peso corporal con Supabase + Realtime
 *
 * Tabla: body_weight_logs
 * - logged_at: timestamptz, fecha y hora del pesaje (autogenerada, editable)
 * - created_at: timestamptz, momento real de inserción (auditoría)
 */
...
export const useWeightStore = create<WeightStore>((set, get) => ({
  ...
  addWeight: async ({ weight_kg, logged_at }) => {
    try {
      ...
      throw err; // que GymPage no cierre el modal si no se guardó
    }
  },
  ...
  updateWeight: async (id, data) => {
    try {
      ...
      set((state) => ({
        weights: sortByLoggedAt(
          state.weights.map((w) => (w.id === id ? { ...w, ...data } : w))
        ),
      }));
    } catch (err) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'UPDATE') {
            ...
            set((state) => ({
              weights: sortByLoggedAt(
                state.weights.map((w) => (w.id === row.id ? row : w))
              ),
            }));
          }
          if (payload.eventType === 'DELETE') {
            ...
            set((state) => ({
              weights: state.weights.filter((w) => w.id !== deletedId),
            }));
          }
        }
      )
